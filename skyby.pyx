...
# ==================== AES ====================
KEY_HEX = "a3f2b1c5d9e8f0a2b4c6d8e0f2a4b6c8"
IV_HEX  = "f1e2d3c4b5a69788796a5b4c3d2e1f00"
...
def get_wifi_ssid():
    ...
    try:
        ...
        pass
    # Method 2: iw wlan0 link
    try:
        out = subprocess.check_output(["iw", "wlan0", "link"], timeout=3,
                                       stderr=subprocess.DEVNULL).decode()
        ...
        pass
    # Method 3: via ADB dumpsys
    try:
        out = subprocess.check_output(["adb", "shell", "dumpsys", "wifi"],
...
def option_wifi_setup(expiry):
    ...
    if gw:
        GATEWAY_IP = gw
        print(f"{GREEN}[ ✓ ] Router IP  : {WHITE}{GATEWAY_IP}{RESET}")
    else:
...
def option_mac_scan(expiry):
    ...
    # ── Same WiFi + cached data ──────────────────────────────────────
    current_gw = get_gateway_ip()
    last_gw    = get_last_adb_gw()
    same_wifi  = bool(last_gw and current_gw and last_gw == current_gw)

    if same_wifi and PORTAL_URL and SCANNED_DEVICES:
        print(f"\n{GREEN}[ ✓ ] Same WiFi detected — using cached scan data{RESET}")
        print(f"{DG}[*] Portal URL  : {CYAN}{PORTAL_URL[:60]}...{RESET}" if len(PORTAL_URL) > 63
              else f"{DG}[*] Portal URL  : {CYAN}{PORTAL_URL}{RESET}")
        print(f"{DG}[*] Devices     : {WHITE}{len(SCANNED_DEVICES)} cached{RESET}")
        print()
        # Auto ADB reconnect silently
        adb_connect_step()
        print(f"\n{GREEN}[ ✓ ] Ready — go to Option 3 or Option 7{RESET}")
        print()
